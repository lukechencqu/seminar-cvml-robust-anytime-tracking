...
\begin{abstract}
  ...
  already various approaches, low accuracy and poor robustnes because
  ...
  histogram is used to globaly search the state space for the most
  ...
  shows that is outperforms all common baseline methods which are a
  ...
  error. Furthermore, the tracker is robost against occlusion,
  viewpoint and lighting changes, and various object classes.
\end{abstract}
...
motion of the tracked object. A mesurement model derived from a
...
previous measurement consisting of a point cloud and color information
...
sampling resolution which is dynamicall improved in the important
...
classes, and when there are problems with the color information.
Evaluation data was created by tracking static objects with a moving car in
a local reference and by building object models and measureing their
crispness score.
...
{\centering{\mathrm{\url{http://stanford.edu/~davheld/DavidHeld_files/RSS2014_Poster.pdf}}}\\}

The implementation is easy to setup, configure, and include into other
C++ projects.

The tracking accuracy probided by annealed dynamic histograms and the
