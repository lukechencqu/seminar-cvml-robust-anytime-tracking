...
a problem when the resolution of the sample in the annealed dynamic
histogram is low. For low resolutions, the likelyhood of a color
machting is smaller than for high resolutions. Therefore, we can model
$p(C)$ as function of the sampling resolution:
\begin{equation}
  p(C) = p_c \mathrm{exp}\left( \frac{-r^2}{2\sigma_c^2} \right)
\end{equation}
with sampling resolution $r$, 
